# Command: Plan From Session Context (write to fixes.md)

You are to take the full context of the prior session and formulate a detailed, in-depth plan for another agent to execute. The plan must emphasize passing rich context along, including reasoning, file references, and motivations behind each fix. The output must be written to `.claude/fixes.md`.

## Pre-check (required)
- Read the current contents of `.claude/fixes.md`.
...
## Requirements
- Use the prior session context plus any related notes already in `.claude/fixes.md`.
...
## Output
- Write the final plan to `.claude/fixes.md` (either merged with related content or overwritten if unrelated).
