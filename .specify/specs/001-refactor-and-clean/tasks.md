...
- [ ] T001 Stage and commit all accumulated `.claude/` and docs changes with message `"clean up .claude tooling and stale docs"`
...
- [ ] T003 Fix stale import in `src/components/brew/HerbSelector.tsx` line 8: change `import { InventoryItem } from '@/lib/inventory'` → `import type { InventoryItem } from './types'`
...
- [ ] T006 Delete `src/lib/recipes.ts` (221 lines) AND simultaneously remove all deprecated exports from `src/lib/hooks/queries.ts`: 5 fetchers, 5+ hooks, 3 invalidation helpers, deprecated query keys (~200 lines)
...
- [ ] T008 Remove deprecated prefetch calls (`prefetchInventory`, `prefetchBrew`, `prefetchRecipes`) from `src/app/page.tsx`
...
- [ ] T014 [US1] Move `CharacterArmorData` to `src/lib/types.ts` using the widest definition (with `properties` + `notes` fields)
