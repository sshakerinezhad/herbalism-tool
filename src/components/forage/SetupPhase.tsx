    ...
    <div className="space-y-8">
      ...
      <div className="elevation-raised rounded-lg p-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h3 className="text-sm font-medium text-vellum-400 mb-1">Foraging Sessions</h3>
            <div className="flex items-center gap-3">
              <div className="text-2xl font-bold">
                <span className={sessionsRemaining === 0 ? 'text-red-400' : 'text-bronze-bright'}>
                  {sessionsRemaining}
                </span>
                <span className="text-vellum-400/60">/{maxForagingSessions}</span>
              </div>
              <span className="text-vellum-400/60 text-sm">remaining today</span>
            </div>
          </div>
          {sessionsUsedToday > 0 && (
            <button
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          {totalAllocated > 0 && (
            <button onClick={onClear} className="text-vellum-400 hover:text-zinc-200 text-sm">
              Clear all
            </button>
          )}
        </div>
            ...
            return (
              <div
                ...
                className={`p-4 rounded-lg border-2 transition-all ${
                  isAllocated
                    ? 'elevation-elevated border-bronze-bright/30'
                    : 'elevation-base'
                }`}
              >
                <h3 className="font-semibold">{biome.name}</h3>
                {biome.description && (
                  <p className="text-vellum-400 text-sm mt-1 mb-3">{biome.description}</p>
                )}
                ...
                <div className="flex items-center gap-2 mt-3">
                  {isAllocated && (
                    <button
                      ...
                      className="w-8 h-8 btn-secondary rounded font-bold transition-colors"
                    >
                      −
                    </button>
                  )}
                  ...
                  {isAllocated ? (
                    <span className="w-8 text-center font-bold text-bronze-bright">{allocated}</span>
                  ) : (
                    <span className="text-vellum-400/60 text-sm">No sessions</span>
                  )}

                  <button
                    onClick={() => onIncrement(biome.id)}
                    disabled={!canAllocateMore}
                    className={`${isAllocated ? 'w-8 h-8' : 'px-3 h-8'} btn-primary disabled:bg-zinc-700 disabled:text-vellum-400/60 rounded font-bold transition-colors`}
                  >
                    {isAllocated ? '+' : '+ Add'}
                  </button>
                </div>
        ...
        {biomes.length === 0 && (
          <p className="text-vellum-400/60">No biomes found. Add some in Supabase!</p>
        )}
      </div>
      ...
      {totalAllocated > 0 && (
        <div className="elevation-raised rounded-lg p-4">
          <h3 className="text-sm font-medium text-vellum-400 mb-2">Session Allocation</h3>
          <div className="flex flex-wrap gap-2">
            {Object.entries(biomeAllocations).map(([biomeId, count]) => {
              ...
              return (
                <span key={biomeId} className="px-3 py-1 elevation-raised border border-bronze-muted/30 rounded-full text-sm">
                  {biome.name}: {count}
                </span>
              )
            })}
          </div>
          <p className="text-vellum-400/60 text-sm mt-2">
      ...
      <button
        ...
        className="w-full px-6 py-4 btn-primary disabled:bg-zinc-700 disabled:text-vellum-400/60 rounded-lg font-semibold text-lg transition-colors"
      >
        {sessionsRemaining === 0
